...
DEFAULT_TARGET_WORD = "law"
DEFAULT_TOP_N_WORDS = 3500
DEFAULT_TOP_N_NEIGHBORS = 3
DEFAULT_TOP_N_COOC = 20
DEFAULT_WINDOW_SIZE = 5
DEFAULT_MIN_COOC = 2
DEFAULT_CHOICE = "lemma"
DEFAULT_MATRICES_DIR = "ppmi_matrix/"
DEFAULT_TOP_K_TRAJ = 5
DEFAULT_TOP_K_AREA = 5

STOPWORDS = set()
lemmatizer = WordNetLemmatizer()
WORDNET_AVAILABLE = False
_wordnet_warning_printed = False
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Genere les trajectoires semantiques et visualisations par decennie."
    )
    parser.add_argument("--corpus-dir", default=DEFAULT_CORPUS_DIR)
    parser.add_argument("--target-word", default=DEFAULT_TARGET_WORD)
    parser.add_argument("--top-n-words", type=int, default=DEFAULT_TOP_N_WORDS)
    parser.add_argument("--top-n-neighbors", type=int, default=DEFAULT_TOP_N_NEIGHBORS)
    parser.add_argument("--top-n-cooc", type=int, default=DEFAULT_TOP_N_COOC)
    parser.add_argument("--window-size", type=int, default=DEFAULT_WINDOW_SIZE)
    parser.add_argument("--min-cooc", type=int, default=DEFAULT_MIN_COOC)
    parser.add_argument("--choice", choices=["form", "lemma"], default=DEFAULT_CHOICE)
    parser.add_argument("--matrices-dir", default=DEFAULT_MATRICES_DIR)
    parser.add_argument("--top-k-traj", type=int, default=DEFAULT_TOP_K_TRAJ)
    parser.add_argument("--top-k-area", type=int, default=DEFAULT_TOP_K_AREA)
    parser.add_argument(
        "--show",
        dest="show",
        action="store_true",
...
def transform_tokens(tokens: list[str], choice: str) -> list[str]:
    global _wordnet_warning_printed
    if choice == "form":
        return tokens
    elif choice == "lemma":
        if not WORDNET_AVAILABLE:
            if not _wordnet_warning_printed:
                print("Ressource NLTK 'wordnet' absente, utilisation des formes brutes.")
                _wordnet_warning_printed = True
            return tokens
...
def top_context_words_from_cooc_row(cooc_row, top_words, target_idx, min_cooc=1, top_n=8):
    ranked = np.argsort(cooc_row)[::-1]
    top_context = []
    for idx in ranked:
        if idx == target_idx:
            continue
        if cooc_row[idx] < min_cooc:
            break
        top_context.append((top_words[idx], int(cooc_row[idx])))
        if len(top_context) == top_n:
            break
    return top_context
...
##### CONFIGURATION
    ...
    TOP_N_COOC      = args.top_n_cooc
    WINDOW_SIZE     = args.window_size
    MIN_COOC        = args.min_cooc
    choice          = args.choice
    MATRICES_DIR    = args.matrices_dir
    TOP_K_TRAJ      = args.top_k_traj
    TOP_K_AREA      = args.top_k_area
...
##### MATRICES PAR DÉCENNIE + PPMI
    ...
    target_cooc_by_decade = {}
    cooc_matrices_by_decade = {}
    ppmi_by_decade = {}
    ...
    for decade in decades_available:
        ...
        cooc_matrices_by_decade[decade] = mat_dec
        ...
        if cooc_vec.sum() == 0:
            ...
            continue
        target_cooc_by_decade[decade] = cooc_vec
...
##### PROJECTION DU MOT CIBLE
    ...
    coords_by_decade = {}
    for decade in sorted(target_cooc_by_decade.keys()):
        cooc         = target_cooc_by_decade[decade]
...
##### TRAJECTOIRE DE TOUS LES MOTS DU VOCABULAIRE
    ...
    top_traj_words = [word for word, _ in ranked_trajectories[:TOP_K_TRAJ]]
    top_area_words = [word for word, _ in ranked_trajectories_area[:TOP_K_AREA]]
...
##### SAUVEGARDE DES TRAJECTOIRES
    ...
    with open(traj_txt_path, "w", encoding="utf-8") as f_out:
        ...
        f_out.write(f"\n\n=== CONTEXTES FRÉQUENTS — TOP-{TOP_K_TRAJ} TRAJECTOIRES ===\n")
        f_out.write("(voisins les plus fréquents selon les matrices de co-occurrence)\n\n")
        for word in top_traj_words:
            word_idx = vocab_index[word]
            global_context = top_context_words_from_cooc_row(
                cooc_global[word_idx],
                top_words,
                word_idx,
                min_cooc=MIN_COOC,
                top_n=8,
            )
            f_out.write(f"Mot : {word}\n")
            if global_context:
                global_context_str = ", ".join(f"{ctx} ({count})" for ctx, count in global_context)
                f_out.write(f"  Global : {global_context_str}\n")
            else:
                f_out.write("  Global : aucun voisin frequent\n")
            for decade in active_decades:
                mat_dec = cooc_matrices_by_decade.get(decade)
                if mat_dec is None:
                    continue
                decade_context = top_context_words_from_cooc_row(
                    mat_dec[word_idx],
                    top_words,
                    word_idx,
                    min_cooc=MIN_COOC,
                ...
                if decade_context:
                    decade_context_str = ", ".join(f"{ctx} ({count})" for ctx, count in decade_context)
                    f_out.write(f"  {decade}s : {decade_context_str}\n")
            f_out.write("\n")

        f_out.write(f"\n=== CONTEXTES FRÉQUENTS — TOP-{TOP_K_AREA} AIRES ===\n")
        f_out.write("(voisins les plus fréquents selon les matrices de co-occurrence)\n\n")
...
##### TOP-N_COOC VOISINS PAR CO-OCCURRENCE (pour le plot UMAP)
    top_cooc_by_decade = {}
    for decade in decades_available:
        cooc = target_cooc_by_decade.get(decade)
