...
# Pour chaque décennie, on cherche les N points du nuage PCA global
# les plus proches (distance euclidienne 2D) du point de trajectoire
# de cette décennie. Le fichier TXT conserve aussi les co-occurrences
# brutes à titre informatif.
neighbors = {}
for decade in active_decades:
    dec_pt  = coords_by_decade[decade]          # coordonnées (PC1, PC2) de la décennie
    cooc    = cooc_by_decade[decade]

    # Distance euclidienne de chaque mot du vocabulaire au point de la décennie
...
# txt_output = f"neighbors_cooc_{TARGET_WORD}.txt"
...
#     for decade in active_decades:
#         f.write(f"--- {decade}s ---\n")
#         f.write(f"{'Rang':<6}{'Mot':<25}{'Dist. PCA':>12}{'Co-occ.':>12}\n")
#         f.write("-" * 55 + "\n")
#         for rank, entry in enumerate(neighbors[decade], start=1):
#             f.write(f"{rank:<6}{entry['word']:<25}{entry['dist']:>12.4f}{entry['cooc']:>12}\n")
#         f.write("\n")
#
...
for decade in active_decades:
    ...
    for entry in neighbors[decade]:
        ...
        # Marqueur du voisin dans l'espace PCA
        ...
        # Ligne reliant le point de la décennie à son voisin contextuel
        ax.plot([tx, nx], [ty, ny],
...
ax.set_title(
    ...
    f"{active_decades[0]}s – {active_decades[-1]}s  ·  "
    f"PPMI (fenêtre={WINDOW_SIZE}) | top-{TOP_N_NEIGHBORS} voisins les plus proches par décennie",
...
legend_elements.append(
    Line2D([0], [0], marker="D", color="w",
           ...
           label=f"Top-{TOP_N_NEIGHBORS}")
)
ax.legend(handles=legend_elements, loc="upper left",
          framealpha=0.9, fontsize=9, title="Décennie")

plt.tight_layout()
output = f"trajectoire_cooc_{TARGET_WORD}.jpeg"
